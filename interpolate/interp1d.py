...
try:
    ...
    raise ImportError(
     ...
     '`git clone git@github.com:aliutkus/pytorch-searchsorted.git`.\n'
     ' * Go in the corresponding directory and launch `make`.\n'
     ' * Copy the whole `searchsorted` directory that has been generated '
     'as a subfolder of the present `interpolate` folder.\n')
...
def interp1d(x, y, xnew, cuda=True):
    ...
    y : (N,) or (D, N) Pytorch Tensor
        A 1-D or 2-D tensor of real values. The length of `y` along its last
        dimension must be the same as that of `x`
    xnew : (P,) or (D, P) Pytorch Tensor
        A 1-D or 2-D tensor of real values. `xnew` can only be 1-D if _both_
        `x` and `y` are 1-D. Otherwise, its length along the first dimension
        must be the same as that of whichever `x` and `y` is 2-D.
    cuda : bool
    ...
    assert (v['x'].shape[1] == v['y'].shape[1]
            and (
                ...
                "the same number of row or one of them having only one row.")
    ...
    if not cuda or not torch.cuda.is_available():
        ...
        yq = torch.zeros(D, v['xnew'].shape[-1], device='cpu')
        ...
        for d in range(D):
            ...
            yq[d] = torch.tensor(interp_func(get_row('xnew', d)), device='cpu')
        if reshaped_xnew:
            yq = yq.view(original_xnew_shape)
        return yq

    # the CPU case is handled if needs be. now we switch to the GPU case.
    # moving everything to GPU in case it was not there already (not handling
    # the case things do not fit entirely, user will do it if required.)
    for name in v:
        v[name] = v[name].to('cuda')

    # assuming x are sorted in the dimension 1, computing the slopes for the
    # segments
    is_flat['slopes'] = is_flat['x']
    v['slopes'] = (v['y'][:, 1:]-v['y'][:, :-1])/(v['x'][:, 1:]-v['x'][:, :-1])
    ...
    res = sel('y') + sel('slopes')*(v['xnew'] - sel('x'))
    return res
